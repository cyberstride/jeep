...
var Piton = function(dir,options){
  ...
  crashGracefully();
  this.app = express();
  this.app.settings.set('environment',options.environment);
  options.dir = dir;
...
Piton.run = function(dir, options){
  var piton = new Piton(dir, options);
  piton.init();
  piton.app.listen(piton.app.config.port);
}
...
function autoload(piton){
  ...
  [options.models, options.middleware, options.controllers].forEach(function(folder){
    ...
    piton.app[folder] = Piton.loadDirectory(moduleDir, piton.app, options);
  });
